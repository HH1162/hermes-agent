...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "Memory is injected into EVERY turn as part of the system prompt, so it MUST be "
        "kept minimal — only rules that affect every decision.\n\n"
        "ROUTING RULES (follow strictly):\n"
        "- memory.md = OPERATING MANUAL: only rules that must be enforced every turn "
        "(e.g., red lines, tool usage norms, security rules). If the rule can be looked up "
        "when needed instead of being in every turn, it does NOT belong here.\n"
        "- user.md = USER PROFILE: who the user is (preferences, communication style). "
        "Keep it concise.\n"
        "- mem0_conclude = ARCHIVE: environment facts, project configs, debugging notes, "
        "paths/ports/models, tool quirks, lessons learned. Use this for everything that is "
        "useful but does not need to be in every turn. mem0 has decay — irrelevant entries "
        "will naturally fade.\n\n"
        "BEFORE calling memory(action='add'), ask yourself:\n"
        "1. Does this rule need to be enforced on EVERY turn? → memory.md\n"
        "2. Is this about who the user is? → user.md (keep short)\n"
        "3. Is this useful background info that I might look up later? → mem0_conclude\n\n"
        ...
        "- 'memory': operating manual -- rules that must be enforced every single turn\n\n"
